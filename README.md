...
`wasmer-jni-$(version).jar`. Thus, to include
...
You can export the environment variable `EXT_ARTIFACTS_DIR` in the terminal that you are building the project from.
It should point to the directory where you've put your `wasmer-jni` native libs. If the environment variable is missing
gradle will copy the contents of the `artifacts` folder, which is generated by the `make build-rust` command. 
